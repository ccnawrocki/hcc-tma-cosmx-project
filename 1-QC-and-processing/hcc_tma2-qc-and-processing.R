...
data_dir <- paste(cosmx_dir, "franses-hcc-data/hcc_tma2", sep = "/") # data directory
list.files(paste(data_dir, "flatFiles/hcc_tma2", sep = "/")) # There should be 5 files here
...
#SetupCosMxTx(.DIR = paste(data_dir, "flatFiles", sep = "/"), .NAME = "hcc_tma2") 

# Reading the data into memory (this is a custom function)
tma2 <- ReadCosMxFF(.DIR = paste(data_dir, "flatFiles", sep = "/"), .NAME = "hcc_tma2")

# Setting up the segmentation for easier plotting (this is a custom function)
tma2 <- SegmentationToMetadata(.SLIDE = tma2)

# Making the segmentation readable for QuPath (this is a custom function)
#MakeGeoJsons(.SLIDE = tma2, .out_dir = paste(data_dir, "geojson_segmentations", sep = "/"))

# Adding metadata
coremap <- readxl::read_excel("1-QC-and-processing/hcc_tma_maps.xlsx", 
                              sheet = "tma2-pathology")
tma2$metadata$fov_type <- plyr::mapvalues(x = tma2$metadata$fov, from = coremap$FOV, to = coremap$Inference)
tma2$metadata$fov_type %<>% as.factor() # Making the new column a factor
tma2$metadata$core <- plyr::mapvalues(x = tma2$metadata$fov, from = coremap$FOV, to = coremap$Comments)
fov_type_map <- tma2$metadata |> dplyr::group_by(fov) |> summarise(fov_type = unique(fov_type))
tma2$metadata$core %<>% as.factor() # Making the new column a factor
pt_map <- openxlsx::read.xlsx("1-QC-and-processing/hcc_tma_maps.xlsx", sheet = "tma2-map", cols = 6:8)
tma2$metadata$patient <- plyr::mapvalues(x = tma2$metadata$core, from = pt_map$core, to = pt_map$patient) |> as.factor()
# The following `from` values were not present in `x`: c1, c5, c7, c13, c19, c20, c21, c24
...
FlowCellCellPlot(tma2$metadata, .cellcolor = "core", size = 1, .fovlabel = F) + 
  ggprism::scale_fill_prism() + 
  theme_void() + 
  coord_fixed() + 
  guides(fill = guide_legend(override.aes = list(size = 5)))
FlowCellCellPlot(tma2$metadata, .cellcolor = "patient", size = 1, .fovlabel = F) + 
  ggprism::scale_fill_prism() + 
  theme_void() + 
  coord_fixed() + 
  guides(fill = guide_legend(override.aes = list(size = 5)))
...
sop1 <- FlowCellCellPlot(.METADATA = tma2$metadata, .cellcolor = "nCount_RNA", .fovlabel = F)  + 
  scale_fill_gradient2(low="beige", mid="beige", high="dodgerblue", 
                       limits = quantile(tma2$metadata$nCount_RNA, probs = c(0, 0.975)), 
                       oob = scales::squish) + 
  theme_void() + 
  coord_fixed() + 
  guides(fill = guide_legend(override.aes = list(size = 5)))
sop1 <- FlowCellFOVPlot(.METADATA = tma2$metadata, .fovcolor = "fov_type", .fovlabel = F, .cellplot = sop1) + 
  scale_color_manual(values = fov_type_cmap) + coord_fixed()
sop1

sop2 <- FlowCellCellPlot(.METADATA = tma2$metadata, .cellcolor = "nFeature_RNA", .fovlabel = F) + 
  scale_fill_gradient2(low="beige", mid="beige", high="red", 
                       limits = quantile(tma2$metadata$nFeature_RNA, probs = c(0, 0.975)), 
                       oob = scales::squish) + 
  theme_void() + 
  coord_fixed() + 
  guides(fill = guide_legend(override.aes = list(size = 5)))
sop2 <- FlowCellFOVPlot(.METADATA = tma2$metadata, .fovcolor = "fov_type", .fovlabel = F, .cellplot = sop2) + 
  scale_color_manual(values = fov_type_cmap) + coord_fixed()
sop2

# Plotting background-related statistics on the flowcell, using a custom function
sop3 <- FlowCellCellPlot(.METADATA = tma2$metadata, .cellcolor = "nCount_negprobes", .fovlabel = F)  + 
  scale_fill_gradient2(low="beige", mid="beige", high="magenta", 
                       limits = quantile(tma2$metadata$nCount_negprobes, probs = c(0, 0.975)), 
                       oob = scales::squish) + 
  theme_void() + 
  coord_fixed() + 
  guides(fill = guide_legend(override.aes = list(size = 5)))
sop3 <- FlowCellFOVPlot(.METADATA = tma2$metadata, .fovcolor = "fov_type", .fovlabel = F, .cellplot = sop3) + 
  scale_color_manual(values = fov_type_cmap) + coord_fixed()
sop3

sop4 <- FlowCellCellPlot(.METADATA = tma2$metadata, .cellcolor = "nCount_falsecode", .fovlabel = F) + 
  scale_fill_gradient2(low="beige", mid="beige", high="orange", 
                       limits = quantile(tma2$metadata$nCount_falsecode, probs = c(0, 0.975)), 
                       oob = scales::squish) + 
  theme_void() + 
  coord_fixed() + 
  guides(fill = guide_legend(override.aes = list(size = 5)))
sop4 <- FlowCellFOVPlot(.METADATA = tma2$metadata, .fovcolor = "fov_type", .fovlabel = F, .cellplot = sop4) + 
  scale_color_manual(values = fov_type_cmap) + coord_fixed()
sop4

# Calculating total counts and genes
tma2$metadata$total_counts <- tma2$counts |> colSums()
tma2$metadata$total_features <- (tma2$counts > 0) |> colSums()

## Cell-Level QC ---------------------------------------------------------------
# We will filter cells on total RNA counts, cell area, proportion of counts from background, and log10(counts/positive gene).
before <- ncol(tma2$counts)

# Identifying negative probes and system controls
idx <- grepl(rownames(tma2$counts), pattern = "Negative")
idx2 <- grepl(rownames(tma2$counts), pattern = "SystemControl")
negs <- tma2$counts[idx,]
scontrs <- tma2$counts[idx2,]
rnacts <- tma2$counts[!(idx|idx2),]

# Plotting the distributions of these variables first
tma2$metadata$fov %<>% as.factor() # Converting the fov column to factor for plotting purposes

# total counts
cqcp1 <- ggplot() +
  geom_density_ridges_gradient(data = tma2$metadata, mapping = aes(x = nCount_RNA, y = fov, fill = fov_type)) +  
...
# Area
cqcp2 <- ggplot() +
  geom_density_ridges_gradient(data = tma2$metadata, mapping = aes(x = Area.um2, y = fov, fill = fov_type)) +  
  facet_wrap(.~fov_type, scales = "free") +
  geom_vline(xintercept = c(25, 1000), color = "cyan2") + 
  theme_classic() + scale_fill_manual(values = fov_type_cmap)

# Proportion of counts from background
tma2$metadata$prop_counts_negative <- tma2$metadata$nCount_negprobes/(tma2$metadata$nCount_RNA + tma2$metadata$nCount_negprobes)
cqcp3 <- ggplot() +
  geom_density_ridges_gradient(data = tma2$metadata, mapping = aes(x = prop_counts_negative, y = fov, fill = fov_type)) +  
  facet_wrap(.~fov_type, scales = "free") +
  geom_vline(xintercept = c(0.1), color = "magenta2") + 
  theme_classic() + scale_fill_manual(values = fov_type_cmap)

# Complexity score: log10(counts per positive gene)
tma2$metadata$log10counts_per_gene <- (tma2$metadata$nCount_RNA/tma2$metadata$nFeature_RNA) |> log10()
cqcp4 <- ggplot() +
  geom_density_ridges_gradient(data = tma2$metadata, mapping = aes(x = log10counts_per_gene, y = fov, fill = fov_type)) +  
  facet_wrap(.~fov_type, scales = "free") +
...
qcresults <- cellpassqc(.metadata = tma2$metadata)
tma2$metadata %<>% cbind(qcresults)
tma2$metadata %<>% mutate(passes_qc = ((pass_counts_qc+pass_area_qc+pass_bg_qc+pass_log10cpg_qc) == 4)) 
...
cqcp6 <- SegmentationPlot(.METADATA = tma2$metadata, .fovs = toremove, .cellcolor = "passes_qc", .outlinecolor = "black", ncol = 2) + 
...
tma2$metadata %<>% filter(passes_qc == T)
tma2$metadata %<>% filter(!(fov %in% toremove[-5])) # I do not want to filter out the control fov, and it is only marginally above 25%
tma2$counts <- tma2$counts[,rownames(tma2$metadata)]

# Removing core c22
tma2$metadata %<>% filter(!(core %in% "c22"))
tma2$counts <- tma2$counts[,rownames(tma2$metadata)]

after <- ncol(tma2$counts)
after/before
# [1] 0.8361669
...
above_bg <- (tma2$counts[!(idx|idx2),] |> rowSums() > thresh)
above_bg |> sum()
# [1] 544
...
cellcompcts <- tma2$tx |> dplyr::group_by(fov, target, CellComp) |> tally() |> collect() |> pivot_wider(names_from = CellComp, values_from = n, values_fill = 0)
...
cellcompcts$lower <- cellcompcts$meanprop-(qt(0.975, df = n_distinct(tma2$metadata$fov))-1)*(cellcompcts$sdprop/sqrt(n_distinct(tma2$metadata$fov)))
cellcompcts$upper <- cellcompcts$meanprop+(qt(0.975, df = n_distinct(tma2$metadata$fov))-1)*(cellcompcts$sdprop/sqrt(n_distinct(tma2$metadata$fov)))
idx3 <- cellcompcts[cellcompcts$upper > 0.5, ]$target
outgenes <- idx3
idx3 <- rownames(tma2$counts) %in% idx3

# Subsetting the counts, based on the work from above
tma2$counts <- tma2$counts[!(idx|idx2|idx3),]

# Making sure no genes have 0 counts
zerocountgenes <- (tma2$counts |> rowSums() == 0) |> sum()
outgenes
# character(0)
zerocountgenes
# [1] 0
...
metadata <- dplyr::select(tma2$metadata, c(1:17, 26, 27, 57, 58, 60:71))
tma2so <- CreateSeuratObject(counts = tma2$counts, meta.data = metadata, project = "hcc_tma2")
control <- subset(tma2so, subset = fov_type == "Kidney")
tma2so <- subset(tma2so, subset = fov_type != "Kidney")

# Normalizing
scale_factor <- 1000
tma2so <- NormalizeData(tma2so, normalization.method = "LogNormalize", scale.factor = scale_factor)
control <- NormalizeData(control, normalization.method = "LogNormalize", scale.factor = scale_factor)

# Variable Features
tma2so <- FindVariableFeatures(tma2so, selection.method = "disp", nfeatures = nrow(tma2so))
control <- FindVariableFeatures(control, selection.method = "disp", nfeatures = nrow(control))

# Plotting the variable features
tma2so@assays$RNA@meta.features$gene <- rownames(tma2so@assays$RNA@meta.features) # Adding the gene names to the gene metadata
tma2so@assays$RNA@meta.features$below_bg <- !above_bg[rownames(tma2so)] # Adding whether or not a gene is above background, as defined above
tma2so@assays$RNA@meta.features$mvp.variable %<>% as.logical()
tma2so@assays$RNA@meta.features$below_bg %<>% as.logical()
...
# Making the plot
vfp <- ggplot(data = tma2so@assays$RNA@meta.features) + 
  geom_point(mapping = aes(x=mvp.mean, y=mvp.dispersion, color = mvp.variable, shape = below_bg)) + 
  scale_color_manual(values = c("black", "red")) + 
  ggrepel::geom_text_repel(data = tma2so@assays$RNA@meta.features |> filter(mvp.variable == T), 
...
# Scaling
tma2so <- ScaleData(tma2so, features = VariableFeatures(tma2so))
control <- ScaleData(control, features = VariableFeatures(control))

# PCA
# Using 25 PCs will be plenty
tma2so <- RunPCA(tma2so, features = VariableFeatures(tma2so), npcs = 50)
ElbowPlot(tma2so, ndims = 50)
# Using 15 PCs will be plenty
control <- RunPCA(control, features = VariableFeatures(control), npcs = 50)
ElbowPlot(control, ndims = 50)

# UMAP
tma2so <- RunUMAP(tma2so, dims = 1:25)
DimPlot(tma2so, group.by = "fov_type", raster = F) + scale_color_manual(values = fov_type_cmap) + theme_void()
DimPlot(tma2so, group.by = "core", raster = F) + ggprism::scale_color_prism() + theme_void()
DimPlot(tma2so, group.by = "patient", raster = F, split.by = "patient") + ggprism::scale_color_prism() + theme_void()
FeaturePlot(tma2so, features = "nCount_RNA") + scale_color_viridis_c(option = "turbo") + theme_void()
...
tma2so[["NP"]] <- CreateAssayObject(counts = negs[,colnames(tma2so)])
tma2so[["FC"]] <- CreateAssayObject(counts = scontrs[,colnames(tma2so)])
SaveSeuratRds(object = tma2so, file = "hcc_tma2_final.RDS")

control[["NP"]] <- CreateAssayObject(counts = negs[,colnames(control)])
control[["FC"]] <- CreateAssayObject(counts = scontrs[,colnames(control)])
SaveSeuratRds(object = control, file = "hcc_tma2_control_final.RDS")
